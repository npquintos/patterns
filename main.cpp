...
StateAndData<Cargo> first_step(Cargo cargo) // procedure for state "start"
{
  std::cout << "first step, cargo is " << cargo.x << std::endl;
  cargo.x = 2;
  return std::make_tuple("third", cargo);
}

StateAndData<Cargo> second_step(Cargo cargo) // procedure for state "second"
{
  std::cout << "second step, cargo is " << cargo.x << std::endl;
  cargo.x = 3;
  return std::make_tuple("end", cargo);
}

StateAndData<Cargo> third_step(Cargo cargo) // procedure for state "third"
{
  std::cout << "third step, cargo is " << cargo.x << std::endl;
  cargo.x = 4;
  return std::make_tuple("second", cargo); // "end" marks end of the steps
}
...
int main()
{
    Cargo c;
    c.x = 1;
    StateMachine<Cargo> sm = StateMachine<Cargo>();

    // add the procedures to call for a given "state"
    // This is STEP 4
    sm.add("start", first_step);
    sm.add("second", second_step);
    sm.add("third", third_step);
